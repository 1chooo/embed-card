...
If the app uses `ThemeProvider` from [`next-themes`](https://github.com/pacocoursey/next-themes), import **`ThemedEmbedCard`** from `embed-card/next-themes` so embeds follow the same resolved theme as the rest of the layout (this docs site uses the same pattern in the playground preview). Same props as `EmbedCard`; an explicit `theme.appearance` still overrides when you need it. See [Getting Started](../getting-started) (section **Match the site theme (next-themes)**) and [Custom Rendering → Dark mode and appearance](../custom-rendering#dark-mode-and-appearance).
