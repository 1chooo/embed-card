...
Older releases exposed additional shadow parts (`header`, `footer`, `preview`); those are removed—style `::part(root)` or rely on attributes and CSS variables from [Custom Rendering → Theming reference](../custom-rendering#theming-reference).
