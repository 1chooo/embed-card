...
**Customize:** `theme` (accent, radius, shadow, font, **`appearance`**) controls the **card chrome** (`colorScheme`, borders, gradients) the same as other providers — including when you use **`ThemedEmbedCard`** from `embed-card/next-themes` so the shell tracks the site light/dark toggle. TikTok’s **inside-iframe** chrome is controlled by TikTok; there is no documented iframe flag comparable to X’s Tweet `theme` parameter.
